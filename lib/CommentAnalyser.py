...
class CommentAnalyser:
    ...
    def parse_reaction_comments(self, comment_text):
        ...
        for word in re.split("der|des|dem|beim", comment_text):
            ...
            if text_spaces[0] != "Beifall" and text_spaces[0] != "Heiterkeit" and text_spaces[0] != "Lachen" and "[" not in word: parties.append(text_spaces[0])
            # getting people
            elif text_spaces[0] != "Beifall" and text_spaces[0] != "Heiterkeit" and text_spaces[0] != "Lachen": people.append(word)
        ...
        parties = [party.replace("BÜNDNIS90/DIE", "BÜNDNIS 90/DIE GRUNE") for party in parties]
        parties = [party.replace("BÜNDNISSES90/DIE", "BÜNDNIS90/DIE GRÜNEN") for party in parties]
        ...
        if "Beifall" in comment_text: comment_type = "Beifall"
        elif "Heiterkeit" in comment_text: comment_type = "Heiterkeit"
        else: comment_type = "Lachen" 

        return {"party_involved": parties, "person": people, "original_text": comment_text, "conotation": conotation, "type": comment_type}

    def parse_zuruf(self, comment_text):
        #Actually parsing Zuruf is hard, so pushing it back until after alpha release
        pass
    ...
    def parse_single_comment(self, comment_text):
        if comment_text[0] == " ": comment_text = comment_text[1:]
        if re.match("Beifall und Heiterkeit|Heiterkeit und Beifall", comment_text):
            return None #"Edge"
    ...
    def parse_comment(self, comment):
        ...
        comment = comment.replace(")", "")
        if "–" in comment:
            # splitting on – by its unicode char which is super weird workaround
            simple_comments = re.split(u"\u2013", comment)
            return [self.parse_single_comment(comment) for comment in simple_comments]
        else: 
